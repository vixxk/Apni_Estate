import math from "mathjs"; // Or just use standard Math

export const calculateEstimate = (req, res) => {
  try {
    const { location, tier = "Tier 2", plotSize, floors = 1, quality = "Standard", 
            inc_labor, inc_plumbing, inc_electrical, inc_waterproof, inc_architect } = req.body;
    ...
    // Assuming 100% coverage as per python default for simplicity unless passed
    // But since req.body might not have coverage logic yet, we just assume:
    // "Usually people build on 80-90% of plot, but for simple math we often assume 100%"
    // The python code asked for coverage. Let's assume 100% calculation if not provided explicitly?
    // Actually the python code does: area_per_floor = plot_size * coverage_percent
    // Let's stick to the prompt's final logic: "area = total_built_up_area"
    // So if the user passes 'area', we use that. If they pass plotSize + floors, we calc.
    // The prompt's python code calculates: total_built_up_area = plot_size * coverage_percent * num_floors.
    // It defaults coverage to 1.0 if not provided.
    
    // Let's implement receiving 'area' directly OR calculating it.
    // But to be safe and match the inputs:
    const coveragePercent = 1.0; 
    const area = safePlotSize * coveragePercent * safeFloors;

    const qMult = quality_multipliers[quality] || 1.0;
    const currentTier = ["Tier 1", "Tier 2", "Tier 3"].includes(tier) ? tier : "Tier 2";

    // Rate helper
    const getRate = (item) => (typeof rates[item] === "object" ? rates[item][currentTier] : rates[item]);
    ...
    const cost_doors = (rates.door_main + (num_doors - 1) * rates.door_internal) * qMult;
    ...
    // contingency
    const cont_percent = (currentTier === 'Tier 1' || quality === 'Premium') ? 0.10 : 0.05;
    ...
    res.status(500).json({ success: false, message: "Estimation failed", error: error.message });
  }
};
